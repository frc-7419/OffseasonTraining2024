...
public class DistanceSensorCommand extends Command {
  ...
  public DistanceSensorCommand(ArmSubsystem armSubsystem, XboxController xboxController) {
  ...
  public void execute() {
    ...
    if (xboxController.getAButtonPressed() && distMXP.getRange()>=0.2) { //not sure what the range is for dist xmp
      armSubsystem.runMotor(0.9);
    }else if(xboxController.getBButtonPressed() &&  distMXP.getRange()>=0.2){
      armSubsystem.runMotor(-0.9);
    }else{
      armSubsystem.brake();
    }
