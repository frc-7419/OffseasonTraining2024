// Copyright (c) FIRST and other WPILib contributors.
// Open Source Software; you can modify and/or share it under the terms of
// the WPILib BSD license file in the root directory of this project.

package frc.robot.commands;

import edu.wpi.first.wpilibj.XboxController;
import edu.wpi.first.wpilibj2.command.Command;
import frc.robot.subsystems.SwerveDrivebaseSubsystem;

public class SwerveDriveFieldCentric extends Command {
  /** Creates a new SwerveDriveFieldCentric. */
  private SwerveDrivebaseSubsystem drivebase;
  private XboxController controller;
  public SwerveDriveFieldCentric(SwerveDrivebaseSubsystem drivebase) {
    this.drivebase = drivebase;
    this.controller = new XboxController(1);
    addRequirements(drivebase);
    // Use addRequirements() here to declare subsystem dependencies.
  }

  // Called when the command is initially scheduled.
  @Override
  public void initialize() {
    drivebase.coast();
    drivebase.resetYaw();
  }

  // Called every time the scheduler runs while the command is scheduled.
  @Override
  public void execute() {
    double forwardsMovement = -controller.getLeftY();
    double rotation = controller.getLeftX();
    drivebase.setRotSpeed(rotation);
    drivebase.setTransSpeed(forwardsMovement);
  }

  // Called once the command ends or is interrupted.
  @Override
  public void end(boolean interrupted) {}

  // Returns true when the command should end.
  @Override
  public boolean isFinished() {
    return false;
  }
}
